...
def delete_team():
    ...
    df = pd.DataFrame(all_teams,columns = ['Team Id', 'Team Name', 'Captain', 'No of players', 'Sports Id', 'Coach Id', 'Match Id', 'League Id'])
    with st.expander("Current data"):
        st.dataframe(df)
    ...
    if st.button("Delete selected Team"):
        ...
        df2 = pd.DataFrame(after_delete, columns = ['Team Id', 'Team Name', 'Captain', 'No of players', 'Sports Id', 'Coach Id', 'Match Id', 'League Id'])
        with st.expander("After deleting the selected Team"):
            st.dataframe(df2)
            
def delete_player():
    all_players = view_all_players()
    df = pd.DataFrame(all_players,columns = ['Player Id', 'Firstname', 'Lastname', 'Date of birth', 'Age', 'Phone no', 'Address', 'Coach Id', 'Team Id'])
    with st.expander("Current data"):
        st.dataframe(df)
    ...
    if st.button("Delete selected Player"):
        ...
        df2 = pd.DataFrame(after_delete, columns = ['Player Id', 'Firstname', 'Lastname', 'Date of birth', 'Age', 'Phone no', 'Address', 'Coach Id', 'Team Id'])
